...
class SystemTrayNotifier extends _$SystemTrayNotifier with TrayListener, AppLogger {
  ...
  Future<void> onTrayMenuItemClick(MenuItem menuItem) async {
    ...
    } else if (menuItem.key == 'renew') {
      // Show window and trigger plans sheet
      await ref.read(windowNotifierProvider.notifier).show();
      // Set a flag so the UI can pick it up and show plans
      _pendingRenew = true;
  ...
  bool _pendingRenew = false;
  bool consumePendingRenew() {
    if (_pendingRenew) {
      _pendingRenew = false;
      return true;
    }
