...
/// Flag to prevent _postSplashTasks and autoSubImport from racing.
/// Set to true once _postSplashTasks starts subscription import.
bool postSplashSubImportStarted = false;
...
Future<void> lazyBootstrap(WidgetsBinding widgetsBinding, Environment env) async {
  ...
  // --- Parallel init: logs, profile repo, translations have no dependencies ---
  await Future.wait([
    _init("logs repository", () => container.read(logRepositoryProvider.future)),
    _init("profile repository", () => container.read(profileRepositoryProvider.future)),
    _init("translations", () => container.read(translationsProvider.future)),
  ]);
  ...
  // --- Parallel init: active profile + hiddify-core have no dependency on each other ---
  await Future.wait([
    _safeInit("active profile", () => container.read(activeProfileProvider.future), timeout: 1000),
    _init("hiddify-core", () => container.read(hiddifyCoreServiceProvider).init()),
  ]);
  ...
  // --- Post-splash tasks: subscription import & bootstrap nodes ---
  // These run AFTER the UI is visible, so they don't block startup.
  unawaited(_postSplashTasks(container, authFuture));
}

/// Heavy network/IO tasks that run after the splash screen is removed.
/// The user sees the home page while these complete in the background.
Future<void> _postSplashTasks(ProviderContainer container, Future<String?> authFuture) async {
  try {
    // Wait for auth result — auth was started at the very beginning of bootstrap,
    // so it's likely already done. Use a short timeout: if it's not ready by now,
    // the network is slow and we shouldn't wait.
    ...
    postSplashSubImportStarted = true;
    if (_pendingSubscriptionUrl != null) {
      await _safeInit("auto-add subscription", () async {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
          ...
          if (added) break;
          // Wait before retry: 1s, 2s (reduced from 2s, 4s)
          if (attempt < 3) {
    ...
    Logger.bootstrap.error("post-splash tasks error", e);
  }
}
